...
func main() {
	...
	keyfilepath := path.Dir(configFile) + "/.zabpsql.key"
	...
	log.Info("Zabpsql agent started pid ", os.Getpid())
	defer log.Info("Zabpsql agent shutdown", os.Getpid())
	//log.Debug("cfg ", cfg)
	...
	for d := range cfg.DBCFG {
		...
		connectString := "postgres://" + dbuser + ":" + dbpass + "@" + cfg.DBCFG[d].ConnectSttring
	...
	go func() {
		...
		log.Info("Zabpsql agent shutdown ", os.Getpid())
		os.Exit(1)
	}()
	for {
...
func handleRequest(conn net.Conn) {
	...
	switch {
	case item == "zabpsql.ping":
		value = "1"
	case item == "zabpsql.version":
		value = myVERSION
	case item == "zabpsql.reqcount":
		value = strconv.Itoa(reqCount.Value())
	case item == "zabpsql.errcount":
		value = strconv.Itoa(errCount.Value())
	case strings.HasPrefix(item, "psql["):
...
func getDBValue(item string) (string, error) {
	...
	value := zabNotSupported + " Unsupported item key. " + item
	item = strings.Replace(item, "psql[", "", 1)
